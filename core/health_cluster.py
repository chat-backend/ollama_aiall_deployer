...
#!/usr/bin/env python3
"""
Cluster health & auto-heal module for AIALL Gateway (Ollama 0.30.x).
...
def curl_health(backend: str, timeout: int = 3) -> bool:
    ...
    - `ollama ps` kiểm tra daemon local.
    - Module này chạy trên node local, nên chỉ có ý nghĩa với backend local.
    """
    try:
